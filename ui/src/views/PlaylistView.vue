...
const savedMatch = computed(() =>
  savedPlaylists.value.find(p => p.name === playlist.value?.title)
)
...
async function onSelectSpeaker(opts: PlayOptions) {
  ...
  if (playAll.value && playlist.value) {
    await player.playYtPlaylist(route.params.playlistId as string, device, { shuffle, repeat }, {
...
<template>
  <div class="playlist-view">
    ...
    <template v-else-if="playlist">
      <!-- Hero header -->
      <div class="hero">
        <div class="hero-bg" :style="playlist.thumbnail ? { backgroundImage: `url(${playlist.thumbnail})` } : {}"></div>
        <div class="hero-content">
          ...
          <div class="hero-body">
            <img
              ...
              @error="onImgError"
            />
            <div class="hero-thumb placeholder img-fallback" :style="{ display: playlist.thumbnail ? 'none' : 'flex' }">
            ...
            <div class="hero-info">
              ...
              <div class="hero-meta">
                ...
                <span class="dot" v-if="playlist.author && playlist.trackCount">&middot;</span>
                <span v-if="playlist.trackCount">{{ playlist.trackCount }} tracks</span>
...
<style scoped>
.playlist-view {
  margin-top: -20px; /* bleed into top */
}
